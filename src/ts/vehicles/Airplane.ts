  ...
  public physicsPreStep(body: CANNON.Body, plane: Airplane): void {
    ...
    if (plane.actions.pitchUp.isPressed) {
      body.angularVelocity.x -=
        right.x * 0.04 * flightModeInfluence * this.enginePower;
      body.angularVelocity.y -=
        right.y * 0.04 * flightModeInfluence * this.enginePower;
      body.angularVelocity.z -=
        right.z * 0.04 * flightModeInfluence * this.enginePower;
    }
    ...
    if (plane.actions.yawLeft.isPressed) {
      body.angularVelocity.x +=
        up.x * 0.02 * flightModeInfluence * this.enginePower;
      body.angularVelocity.y +=
        up.y * 0.02 * flightModeInfluence * this.enginePower;
      body.angularVelocity.z +=
        up.z * 0.02 * flightModeInfluence * this.enginePower;
    }
    ...
    if (plane.actions.rollLeft.isPressed) {
      body.angularVelocity.x -=
        forward.x * 0.055 * flightModeInfluence * this.enginePower;
      body.angularVelocity.y -=
        forward.y * 0.055 * flightModeInfluence * this.enginePower;
      body.angularVelocity.z -=
        forward.z * 0.055 * flightModeInfluence * this.enginePower;
    }
    ...
    if (plane.actions.throttle.isPressed && !plane.actions.brake.isPressed) {
      ...
      !plane.actions.throttle.isPressed &&
      plane.actions.brake.isPressed
    ) {
      speedModifier = -0.05;
    ...
    body.velocity.x +=
      (velLength1 * this.lastDrag + speedModifier) *
      forward.x *
      this.enginePower;
    body.velocity.y +=
      (velLength1 * this.lastDrag + speedModifier) *
